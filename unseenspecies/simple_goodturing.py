#!/usr/bin/env python
#
# Taken from cellranger, slight extension
...
def sgt_proportions_adjusted_for_freqfreq(freqfreqs_dict: dict):
    """
    basically the same as sgt_propotions, but operating on the frequencies of frequencies already!
    adjusted the proporions in the freqfreq to account for unseen species
    """
    assert np.all(np.array(list(freqfreqs_dict.keys())) > 0)
    # assert len(use_freqs) == len(freqfreqs)

    if len(freqfreqs_dict.keys()) < 10:
        raise SimpleGoodTuringError(
            "Too few non-zero frequency items. Aborting SGT."
        )

    use_freqs, freqfreqs = zip(*freqfreqs_dict.items())
    use_freqs = np.array(use_freqs)
    freqfreqs = np.array(freqfreqs)

    ## NOTE: simple_good_turing expects these to be SORTED
    ix = np.argsort(use_freqs)
    use_freqs = use_freqs[ix]
    freqfreqs = freqfreqs[ix]

    rstar, p0 = simple_good_turing(use_freqs, freqfreqs)

    # rstar contains the smoothed frequencies.
    # Map each original frequency r to its smoothed rstar.
    rstar_dict = dict(zip(use_freqs, rstar))

    rstar_sum = np.sum(freqfreqs * rstar)

    import toolz
    p_adj_freq_freq = toolz.valmap(lambda x: (1-p0) *x/rstar_sum, rstar_dict)


    assert np.isclose(
        sum(freqfreqs_dict[n] *  p_adj_freq_freq[n]  for n in use_freqs) + p0,
    1)
    return p_adj_freq_freq, p0
